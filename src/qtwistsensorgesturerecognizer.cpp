...
bool QTwistSensorGestureRecognizer::start()
{
    ...
    if (QtSensorGestureSensorHandler::instance()->startSensor(QtSensorGestureSensorHandler::Gyroscope)) {
        ...
        qDebug() << "Could not start twist gesture: Orientation";
        } else {
...
void QTwistSensorGestureRecognizer::rotationChanged(QRotationReading *reading)
  {
...
//        dataList.removeFirst();
    ...
    qreal angle = reading->y();//qAtan(x / qSqrt(y*y + z*z)) * RADIANS_TO_DEGREES;
...
{
    currentReading.setX(reading->x());
    currentReading.setY(reading->y());
    currentReading.setZ(reading->z());
...
void QTwistSensorGestureRecognizer::timeout()
{
    if (orientationReading)
        qDebug() << /*currentReading.x() << */currentReading.y() /*<< currentReading.z() */<< orientationReading->orientation();
